...
## College Campus {#ref-campus}


```{r}
#| fig-height: 6
#| fig-width: 8
#| fig-cap: !expr sprintf("%s Survey Participants by Campus %s.", NextFigure(), print_n_reporting('school'))

# come back to this with Robin. -- must 
ggplot(tmp, aes(x=school2, fill = sector_color)) + geom_bar() + coord_flip() + 
  labs(x = "School", y = "Number of Students") + scale_fill_manual(values = c('#4895EF', 'gold', '#9D2235'), name = 'School Types', labels = c('UC', 'CCC', 'CSU'))
...
**Summary: ** A total of `r count_and_percent(tmp$sector, "CSU")`, `r count_and_percent(tmp$sector, "CCC")` and `r count_and_percent(tmp$sector, "UC")` college students were enrolled in CSU, CCC and UC campuses, respectively. All college students were either in the Educational Opportunity Program (EOP) at the UC or CSU systems or in the Extended Opportunity Programs and Services (EOPS) in the CCC systems.
...
plot_frq(bns$q15, geom.colors = plot_color, wrap.labels = 8, ylim=c(0, 20)) + xlab('Class Standing') + 
...
plot_frq(bns$q16, geom.colors = plot_color, ylim=c(0,25), wrap.labels = 16, sort.frq = 'desc') + xlab('') + ylab("Number of Students") + theme(axis.text.x = element_text(size = 10))
