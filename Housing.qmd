...
plot_frq(bns$q24, geom.colors = plot_color, ylim=c(0,50)) + xlab('') + ylab("Number of Students")
```
:::
...
plot_frq(bns$q34, geom.colors = plot_color, wrap.title=40, wrap.labels =12, ylim=c(0,40)) + xlab('') + ylab("Number of Students")
...
plot_frq(bns$q37, geom.colors = plot_color, wrap.labels = 17, ylim=c(0,35), sort.frq = 'desc') + xlab('') + ylab("Number of Students") + theme(axis.text.x = element_text(size = 10))
```
...
plot_frq(bns$q23, geom.colors = plot_color, ylim=c(0,50)) + xlab('') + ylab("Number of Students")
...
**Summary:** Of those who reported a change in housing stability from the [COVID-19 Housing Impact](#COVID_Housing_Impact) section, `r get_count_and_percent(bns$covid_housing_off_campus_job, "1")` students reported that the reason for the change in their housing stability was because their finances changed due to a lost (or reduced) OFF-campus job, while `r get_count_and_percent(bns$covid_housing_campus_job, "1")` reported their finances changed due to a lost (or reduced) ON-campus job. Additionally, `r get_count_and_percent(bns$covid_housing_unsafe, "1")` reported that they were in unsafe housing, and `r get_count_and_percent(bns$covid_housing_roommate_finances, "1")` reported that their roommate's finances had changed. 
