...
huxtable(tmp, add_rownames = "") %>% set_background_color(2:12, 2:8, "tomato") %>%
  ...
  set_background_color(2:9, 8, "darkolivegreen3") %>% set_background_color(2:10, 8, "darkolivegreen3") %>%
  set_background_color(11:12, 8, "yellow") %>%
  set_bold(2:3, 2) %>% set_bold(2:4, 3) %>% set_bold(2:5, 4) %>% set_bold(2:6, 5) %>% set_bold(2:7, 6) %>% 
  set_bold(2:8, 7) %>%  set_bold(2:9, 8) %>% set_bold(2:10, 8) %>% 
  set_italic(4:10, 2) %>% set_italic(5:10, 3) %>% set_italic(6:10, 4) %>% set_italic(7:10, 5) %>% 
  set_italic(8:10, 6) %>% set_italic(9:10, 7) %>%  set_italic(10, 8) %>% set_italic(11, 8) %>%
...
plot_frq(c1$`ifelse(bns.calfresh_eligible == 0, "No", "Yes")`, geom.colors = plot_color,  ylim=c(0,45)) + 
...
#| fig-width: 9
p1 <- plot_frq(bns$q47, geom.colors = plot_color, wrap.title = 40, ylim=c(0,10), wrap.labels = 15, y.offset = y.off(bns$q47)) +
  xlab('\nDid you apply for CalFresh EBT as an individual \nor with other people in your household?') + ylab("Number of Students")
p2 <- plot_frq(bns$fed_through_EBT, geom.colors = plot_color, wrap.title = 40, y.offset = y.off(bns$fed_through_EBT)) + 
  xlab('\nHow many people in the household are regularly \nfed through your CalFresh EBT card?')
...
tmp[4,] <- list("", "Total Average", round(mean(tmp$HH.avg),2))
...
p3 <- plot_frq(bns$q50_tmp, geom.colors = plot_color, ylim=c(0,5), y.offset = y.off(bns$q50)) + 
...
p4 <- plot_frq(bns$q51, geom.colors = plot_color, ylim=c(0, 5), y.offset = y.off(bns$q51), wrap.labels = 12) + 
